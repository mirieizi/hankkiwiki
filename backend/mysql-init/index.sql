...
-- food 테이블 최적화 인덱스들
CREATE UNIQUE INDEX idx_food_name_unique ON food(food_name);
CREATE INDEX idx_food_major_category ON food(major_category);
CREATE INDEX idx_food_sub_category ON food(sub_category);

ALTER TABLE food ADD FULLTEXT INDEX ft_food_name (food_name);

-- 복합 인덱스 (카테고리 조합 검색용)
CREATE INDEX idx_food_categories ON food(major_category, sub_category);

-- 영양소 검색용 인덱스들
CREATE INDEX idx_food_kcal ON food(kcal);
CREATE INDEX idx_food_protein ON food(protein);
CREATE INDEX idx_food_carbohydrate ON food(carbohydrate);
CREATE INDEX idx_food_fat ON food(fat);

-- 영양소 범위 검색용 복합 인덱스 (고단백 저칼로리 등)
CREATE INDEX idx_food_nutrition_combo ON food(kcal, protein, carbohydrate, fat);

-- 서빙 사이즈 검색용
CREATE INDEX idx_food_serving_size ON food(serving_size);
